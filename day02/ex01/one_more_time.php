...
function	test_hour($hour)
{
	...
	{
		$flag = 1;
		if (preg_match("/^0[0-9]|1[0-9]|2[0-4]$/", "$hour[0]") == 0)
			$flag = 0;
		if (preg_match("/^0[0-9]|[1-5][0-9]$/", "$hour[1]") == 0) /* 60 min invalide */ 
			$flag = 0;
		if (preg_match("/^0[0-9]|[1-5][0-9]$/", "$hour[2]") == 0) /* 60 secondes idem  */
			$flag = 0;
		return ($flag);
	}
}
...
if ($argc > 1)
{
	...
	else
	{
		...
		if ($result == 0)
            $flag = 1;
        if ($result == 2 && $tab[1]>29)
            $flag = 1;
		if (preg_match("/^[0-9]{4}$/", "$tab[3]") == 0)
			$flag = 1;
		if($tab[3] < 1970)
		{
			echo "année inferieure à 1970\n";
			exit();
		}	
		...
		if ($flag == 0)
		{
			...
			$time = (int)mktime($hour[0], $hour[1], $hour[2], $result, $tab[1], $tab[3]);
			echo "$time\n";
		}
		else
			echo "Wrong Format\n";
